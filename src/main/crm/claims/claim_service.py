...
class ClaimService:
    ...
    def try_to_automatically_resolve(self, claim_id: int) -> Claim:
        ...
        claims_resolver = self.find_or_create_resolver(claim.owner_id)
        transits_done_by_client: List[TransitDetailsDTO] = self.transit_details_facade.find_by_client(claim.owner_id)
        client_type: Client.Type = self.client_repository.get_one(claim.owner_id).type
        ...
        if result.decision == Status.REFUNDED:
            claim.refund()
            self.client_notification_service.notify_client_about_refund(claim.claim_no, claim.owner_id)
            if client_type == Client.Type.VIP:
                self.awards_service.register_non_expiring_miles(claim.owner_id, 10)
        ...
        if result.who_to_ask == ClaimsResolver.WhoToAsk.ASK_CLIENT:
            self.client_notification_service.ask_for_more_information(claim.claim_no, claim.owner_id)
        return claim

    def find_or_create_resolver(self, client_id: int) -> ClaimsResolver:
        resolver = self.claims_resolver_repository.find_by_client_id(client_id)
        if resolver is None:
            resolver = self.claims_resolver_repository.save(ClaimsResolver(client_id=client_id))
        return resolver
